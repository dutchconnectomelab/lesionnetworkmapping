function plot_lh_surface(data, varargin)
    % PLOT_RH_SURFACE Displays parcel-wise data on the fsaverage left hemisphere 
    % by mapping Schaefer1000 parcel values to vertices and rendering them on
    % lh.inflated. Accepts vectors of length 500 (RH only), 1000 (LH+RH; RH part is used), 
    ...
    %
    % Output:
    %   Creates a figure with the left-hemisphere surface colored
    ...
    %       - annotation/lh.schaefer1000-yeo7.annot
    %
    % Example
    %   plot_lh_surface(LNM(501:end), 'title','LNM — RH', 'vmin',-4, 'vmax',4, ...
    ...
    if nParcels==500
        fprintf('[INFO] %d parcels → assuming Schaefer-1000 lh for plotting lh surface.\n',nParcels);
    elseif nParcels==1000
        fprintf('[INFO] %d parcels → assuming lh+lh (Schaefer-1000) for plotting lh surface.\n',nParcels);
        data = data(1:500);
    elseif nParcels==1054
       fprintf('[INFO] %d parcels → assuming Melbourne54+lh+lh (Schaefer-1000) for plotting lh surface.\n',nParcels);
        data = data(55:554);
    else
    ...
    %% plot surface
    % load surfaces
    [lh_ver, lh_faces] = read_surf(fpath(projectDir,'lh.inflated', 'mustContain', 'fsaverage7'));
    
    % load annotations
    lh_annot_path = fpath(projectDir,'lh.schaefer1000-yeo7_fs7.annot');
    
    [~,~,lh_ct] = read_annotation(lh_annot_path, 0);
    
    % skip medial wall
    lh_rois = lh_ct.struct_names(2:end);
    
    % project to surface
    lh_data = schaefer2fsaverage(data, lh_annot_path, lh_rois, 'lh');
    
    % plot surface (pass ax through)
    plot_surface(lh_ver, lh_faces, lh_data, 'cmap', cmap, 'vmin', vmin, 'vmax', vmax, ...
                 'vcenter', vcenter, 'azimuth', 0, 'elevation', 270, 'ax', ax, 'cbar', cbar)
